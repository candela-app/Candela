...
- Pick **one module**. Optionally pick a **level**; leave Level on All levels for the whole module. Efficiency (Index of Performance) is the speed–accuracy line when Accuracy and RT disagree. **Time to finish** is mean sitting length in that bucket (best = shortest). Same module, level, and From–To filters as the other charts — not avg reaction time.
