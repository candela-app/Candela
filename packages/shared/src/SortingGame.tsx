...
import { SortingVariant, BubbleItem, BubblePosition } from './types';
import { ALPHABETS, NUMBERS, THERAPY_COLORS } from './constants';
import {
  checkOverlap,
  getContrastColor,
  exportSessionCSV,
...
/**
 * Shared Sorting Game Component for Vision Therapy & Visual Discrimination.
 * Standardized template, responsive batching (5-5-5-5-6 desktop, 4-4-4-4-4-4-2 mobile/tabs),
 * pitch-black anti-glare canvas, shared audio-haptics, miss-press detection, and ClinicalSettingsModal.
 */
export function SortingGame({ variant = 'uppercase', onExit }: SortingGameProps) {
  ...
  const handleBubbleClick = (clickedBubble: BubbleItem) => {
    ...
    const allItems = sequenceItems();
    const expectedSymbol = allItems[expectedIndex];

    if (clickedBubble.symbol === expectedSymbol) {
      playCorrectSoundAndHaptic();
      ...
      setTimeout(() => {
        setBubbles((prev) => prev.filter((b) => b.id !== clickedBubble.id));
        setPoppingIds((prev) => {
        ...
        if (nextIndex < allItems.length) {
          const plan = getBatchPlan();
          let itemsInBatchesSoFar = 0;
          for (let i = 0; i <= currentBatchIndex && i < plan.length; i++) {
            itemsInBatchesSoFar += plan[i];
          }

          if (nextIndex >= itemsInBatchesSoFar) {
            const nextBatchIdx = currentBatchIndex + 1;
            setCurrentBatchIndex(nextBatchIdx);
            spawnBatch(nextBatchIdx, allItems);
          }

          const nextTarget = allItems[nextIndex];
          if (nextTarget) {
            speak(`target ${nextTarget}`);
          }
        } else {
