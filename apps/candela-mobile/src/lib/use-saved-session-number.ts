...
import { getDeviceTier, payloadFromSessionResult, type SessionResultData } from '@candela/shared/rn';
import { persistGameSession } from '../lib/api';

function currentDeviceTier() {
  const { width, height } = Dimensions.get('window');
  return getDeviceTier(width, height);
}
...
export function useSavedSessionNumber(isOpen: boolean, data: SessionResultData | null | undefined) {
  ...
  useEffect(() => {
    ...
    const payload = payloadFromSessionResult(data, { deviceTier: currentDeviceTier() });
    if (!payload) {
      setStatus('skipped');
      return;
    }
