...
export function LookPursuitGame({
  onExit,
...
function LookPursuitMovingGame({
  ...
  useEffect(() => {
    ...
    const loop = (now: number): void => {
      ...
      if (!sample.faceLost) {
        ...
        const overId = resolveLookOverId(lookX, lookY, targetStateRef.current, decoyStatesRef.current, settings.bubbleSizePx);
        const fired = advanceLookDwell(dwellRef.current, overId, dt * 1000);
        if (fired) {
  ...
  return (
    ...
    >
      ...
      {gameStarted && faceLost && !lookError ? (
        <Text style={{ position: 'absolute', top: s(72), alignSelf: 'center', color: '#FDE68A', fontWeight: '700' }}>
          Face the camera
        </Text>
      ) : null}
